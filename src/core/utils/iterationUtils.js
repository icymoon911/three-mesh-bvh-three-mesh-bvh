...
export function intersectTris( bvh, materialOrSide, ray, offset, count, intersections, near, far ) {
	...
	if ( _indirectBuffer ) {
		...
		for ( let i = offset, end = offset + count; i < end; i ++ ) {
			...
			intersectTri( geometry, materialOrSide, ray, _indirectBuffer[ i ], intersections, near, far );
...
export function intersectClosestTri( bvh, materialOrSide, ray, offset, count, near, far ) {
	...
	if ( _indirectBuffer ) {
		...
		for ( let i = offset, end = offset + count; i < end; i ++ ) {
			...
			const intersection = intersectTri( geometry, materialOrSide, ray, _indirectBuffer[ i ], null, near, far );
		...
		for ( let i = offset, end = offset + count; i < end; i ++ ) {
			...
			const intersection = intersectTri( geometry, materialOrSide, ray, i, null, near, far );
			if ( intersection && intersection.distance < dist ) {
