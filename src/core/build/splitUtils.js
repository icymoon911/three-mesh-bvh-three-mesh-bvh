...
function centerSplit( nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count ) {
...
function averageSplit( nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count ) {
...
function sahSplit( nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count ) {
...
/**
 * Registry of split strategies. Each strategy is a function with signature:
 * `(nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count) => { axis, pos }`
 *
 * New strategies can be registered via `registerSplitStrategy`.
 */
const _strategies = new Map();
_strategies.set( CENTER, centerSplit );
_strategies.set( AVERAGE, averageSplit );
_strategies.set( SAH, sahSplit );

/**
 * Register a custom split strategy.
 * @param {number|string} id - Unique identifier for the strategy (can be a custom constant).
 * @param {Function} strategyFn - Function with signature
 *   `(nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count) => { axis, pos }`.
 */
export function registerSplitStrategy( id, strategyFn ) {

	_strategies.set( id, strategyFn );

}

/**
 * Get the split strategy function for the given id.
 * @param {number|string} id
 * @returns {Function|null}
 */
export function getSplitStrategy( id ) {
...
export function getOptimalSplit( nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count, strategy ) {
	...
	const strategyFn = _strategies.get( strategy );
	if ( ! strategyFn ) {
	...
	return strategyFn( nodeBoundingData, centroidBoundingData, primitiveBounds, offset, count );
